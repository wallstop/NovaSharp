...
- Progress snapshot (2025-11-29):
  ...
  - 2025-11-30 (later): Removed the legacy NUnit fixtures for user-data fields/properties/indexers/events plus the unit-level `UserDataTests` now that their TUnit counterparts are stable; regenerated `FixtureCatalogGenerated.cs` (233 fixtures) and confirmed `dotnet test --project src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --no-build --settings scripts/tests/NovaSharp.Parallel.runsettings` still passes (**3,159** tests). Continue deleting the remaining duplicated NUnit fixtures (methods/overloads/nested types, CLI shells, TAP/spec harnesses) in small batches so the catalog diff stays reviewable.
  - 2025-11-30 (evening): Deleted the remaining user-data NUnit fixtures (enums/methods/overloads/nested-types/meta); `FixtureCatalogGenerated.cs` now tracks **228** fixtures and the interpreter NUnit suite still succeeds (`3016` tests). Next bucket: finish removing the `VtUserData*` legacy fixtures plus the CLI TAP/spec suites once their TUnit equivalents are in place, then flip docs/tooling to reference the TUnit host as canonical.
