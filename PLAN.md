...
- Progress snapshot (2025-11-28):
  - Microsoft.Testing.Platform is enforced across NUnit and TUnit projects; the shared adapters in `src/tests/TestInfrastructure` now serve both runners, and compare artefacts remain under `artifacts/tunit-migration/`.
  - Interpreter VM/API suites plus the descriptor-focused coverage (AutoDescribingUserDataDescriptor, FieldMemberDescriptor, PropertyMemberDescriptor, MethodMemberDescriptor, DispatchingUserDataDescriptor, CompositeUserDataDescriptor, ProxyUserDataDescriptor) now run on TUnit—`dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` executes **621** tests in ~2.4 s with zero failures (2025-11-28 23:45 UTC).
  - Descriptor progress: the AutoDescribingUserDataDescriptor, FieldMemberDescriptor, PropertyMemberDescriptor, MethodMemberDescriptor, DispatchingUserDataDescriptor, CompositeUserDataDescriptor, and ProxyUserDataDescriptor NUnit suites have TUnit mirrors under `Descriptors/AutoDescribingUserDataDescriptorTUnitTests.cs`, `Descriptors/FieldMemberDescriptorTUnitTests.cs`, `Descriptors/PropertyMemberDescriptorTUnitTests.cs`, `Descriptors/MethodMemberDescriptorTUnitTests.cs`, `Descriptors/DispatchingUserDataDescriptorTUnitTests.cs`, `Descriptors/CompositeUserDataDescriptorTUnitTests.cs`, and `Descriptors/ProxyUserDataDescriptorTUnitTests.cs`, covering user-data delegation paths plus reflection/preoptimization/AOT guard rails without relying on NUnit’s `Assert.Multiple`.
  - Platform detector coverage: added `Platforms/PlatformAutoDetectorTUnitTests.cs` to mirror the NUnit suite (loader selection, Unity detection overrides, AOT probe overrides/exceptions, cached flag restoration). The latest release run above (2025-11-28 21:05 UTC) executes **691** TUnit tests in ~2.0 s with zero failures.
  - CLI coverage: `Cli/RunCommandTUnitTests.cs`, `Cli/RegisterCommandTUnitTests.cs`, `Cli/CompileCommandTUnitTests.cs`, `Cli/HelpCommandTUnitTests.cs`, `Cli/HardwireCommandTUnitTests.cs`, `Cli/DebugCommandTUnitTests.cs`, `Cli/CliIntegrationTUnitTests.cs`, `Cli/ShellCommandManagerTUnitTests.cs`, and `Cli/ExitCommandTUnitTests.cs` now mirror the NUnit run/register/compile/help/hardwire/debug/shell-integration/command-manager/exit suites, covering interactive prompts, syntax hints, loader selection, manifest-driven compatibility logs, debugger attach flows, command routing, exit-path signaling, user-data registration errors, REPL execution, compile success/failure paths, and hardwire dump generation via shared console-capture + platform-detection helpers so CLI regressions stay visible under Microsoft.Testing.Platform.
  - Module coverage: `Modules/OsTimeModuleTUnitTests.cs` now drives the Lua os.date/os.time/os.clock/os.difftime regression suite under TUnit, hitting the table parsing paths plus SequenceTimeProvider hooks without relying on NUnit.
  - Expanded module parity with `Modules/OsSystemModuleTUnitTests.cs`, mirroring the NUnit coverage for `os.execute`, `os.exit`, `os.getenv`, file delete/rename paths, time/date helpers, and temp-file generation. The new helper wraps platform overrides in `Script.BeginGlobalOptionsScope()` so tests can safely replace `Script.GlobalOptions.Platform` even while the rest of the suite runs in parallel.
  - `Modules/IoModuleVirtualizationTUnitTests.cs` and `Modules/IoModuleTUnitTests.cs` now mirror the NUnit IO coverage: the virtualization suite brings over the in-memory platform accessor while `IoModuleTUnitTests` ports the real-file scenarios (open errors, std stream overrides, `io.lines` default-input + handle iterators, chunked read variants, binary encoding guards, tmpfile behaviors, numeric parsing guard rails, close/flush semantics, stderr safeguards, and `io.popen` error messaging). Both suites rely on the shared global-options scope so their platform swaps and stream overrides stay local to each test.
  - Stream userdata now runs under TUnit: added `Modules/StreamFileUserDataBaseTUnitTests.cs`, a straight port of the 67-case NUnit fixture that revalidates `StreamFileUserDataBase` write/flush/seek/setvbuf/read/lines/hex-literal parsing plus the helper user-data types without relying on NUnit’s `Assert.Multiple`. The new fixture reuses the isolation attributes, wires up a TUnit-friendly initialization helper, and passes the full TUnit suite (`dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` on 2025-11-28) so coverage stays green under Microsoft.Testing.Platform.
  - Loader coverage keeps pace with the module/CLI work: `Loaders/FileSystemScriptLoaderTUnitTests.cs` mirrors the NUnit file-system loader suite (path normalization + null guards), `Loaders/EmbeddedResourcesScriptLoaderTUnitTests.cs` covers resource enumeration plus the .NET Core-specific constructor guard, `Loaders/ReplInterpreterScriptLoaderTUnitTests.cs` exercises the REPL loader’s environment-variable precedence/Lua-path overrides, `Loaders/UnityAssetsScriptLoaderTUnitTests.cs` drives the Unity reflection harness (resource discovery, failure handling, recoverable/non-recoverable exceptions) without NUnit, and `Loaders/InvalidScriptLoaderTUnitTests.cs` keeps the placeholder loader’s pass-through/exception semantics under Microsoft.Testing.Platform. Latest TUnit run (`dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` on 2025-11-28 22:05 UTC) executes **732** tests in ~2.3 s with zero failures.
  - The remote-debugger pilot host (`src/tests/NovaSharp.RemoteDebugger.Tests.TUnit`) now covers handshake/policy requests, watch evaluation, host-state transitions, queued refreshes, breakpoint refresh, pause/run draining, and decorated runtime errors, completing in <0.9 s and giving us a stable, higher-fidelity baseline for future debugger migrations.
  - Platform detector stability (2025-11-29): `PlatformAutoDetector.IsRunningOnAot` now uses a tri-state, `Volatile`-backed cache with a synchronization gate, eliminating the sporadic `Nullable` failures seen when `StringFind8` exercised the IO module under parallel fixture stress. Added the NUnit analogue of the TUnit concurrent-reset stress test so both frameworks now guard the AOT cache against racing probe resets.
...
- **Next steps (2025-11-28 update):**
  1. Inventory the remaining NUnit fixtures (descriptor/hardwire modules, CLI/REPL helpers, EndToEnd/spec suites) and prioritize the next TUnit conversion batches, capturing compare-test-runtimes artefacts for each migration.
  2. Expand the interpreter TUnit host beyond VM/core by porting the remaining module + CLI regression suites (IO/OS modules, remaining CLI command fixtures such as the shell/register integration harnesses, plus script loaders) so the legacy NUnit project can be retired without coverage loss; follow up by retiring the duplicated PlatformAutoDetector/Run/Register/Compile/Help/Hardwire/Debug NUnit fixtures once their TUnit counterparts soak. **Next port:** target the `StreamUserData`/file loader regression fixtures (or the remaining CLI shell/register harnesses) so the rest of the IO-heavy suites can follow the freshly-converted stream userdata tests.
  3. Extend the remote-debugger TUnit coverage (attach, pause, watches, transport errors) and update docs/build scripts to treat TUnit as the canonical runner once the pending conversions are complete.
  4. After the TUnit conversion is complete and the legacy NUnit projects are removed, begin the repository re-organization effort (see “Codebase organization & namespace hygiene”) so the post-migration folder/project layout reflects the cleaner test split.
  5. Eliminate lingering `try`/`finally` clean-up patterns from the test suites (particularly in the remaining NUnit fixtures) by introducing/shared `IDisposable` helpers (`using` scopes for temp files, platform swaps, console capture, etc.) so every test manages resources consistently without manual tear-down blocks.
