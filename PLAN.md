...
- 2025-12-01 18:25 UTC: Introduced `Script.BeginDefaultOptionsScope` plus the shared `ScriptDefaultOptionsScope` helper and converted `ScriptDefaultOptionsTUnitTests` along with `ScriptToClrConversionsTUnitTests.DynValueToObjectUsesCustomConversionResult` to the `using`-based scopes, removing another recurrent `try`/`finally` path.
- 2025-12-01 20:05 UTC: Added `DebugCommandScope`, `UserDataRegistrationScope`, and `SemaphoreSlimScope` helpers, then rewired the CLI integration/register suites to use them instead of bespoke `try`/`finally` blocks (covering debugger overrides, user-data registrations, and Hardwire semaphore gates).
- 2025-12-01 21:10 UTC: Added `EnvironmentVariableScope` plus additional `TempFileScope` usage, then converted the StandardPlatformAccessor unit tests off their environment/temp-file `try`/`finally` blocks so the file-IO coverage now leans solely on the shared scopes.
- 2025-12-01 22:00 UTC: Introduced `UserDataRegistrationPolicyScope` and updated the user-data unit suite to track descriptor/registration policy mutations with scopes (AutomaticRegistrationPolicy/Composite descriptor tests now use the helper, and the custom descriptor cases leverage `UserDataRegistrationScope` instead of manual `try`/`finally` blocks).
- 2025-12-01 22:20 UTC: Reworked `StandardGenericsUserDataDescriptorTUnitTests` to consume `UserDataRegistrationScope`, so both the "already registered" and "generate new type" paths use the shared helper instead of hand-rolled `try`/`finally` cleanup.
- 2025-12-01 22:35 UTC: Updated the `CollectionsRegistered` end-to-end suite so its shared helper method now tracks all descriptor registrations via `UserDataRegistrationScope` instead of manually unregistering lists/arrays in `finally`.
- 2025-12-01 22:45 UTC: Wrapped the custom-converter-heavy `UserDataMethods` regression test (`TestConcatMethodStaticComplexCustomConvAsync`) in `Script.BeginGlobalOptionsScope` so the `try`/`finally` cleanup dropped in favor of the new scope.
- 2025-12-01 22:55 UTC: Converted `UserDataPropertiesTUnitTests.WithRegistrationAsync` to a `UserDataRegistrationScope`, so property registration helpers no longer call `UserData.UnregisterType` inside `finally`.
- 2025-12-01 23:05 UTC: Applied the same `UserDataRegistrationScope` pattern to `UserDataFieldsTUnitTests.WithRegistrationAsync`, removing another helper-level `try`/`finally`.
- 2025-12-01 23:15 UTC: `UserDataOverloadsTUnitTests.InteropOutParamInOverloadResolution` now uses `UserDataRegistrationScope` for the dictionary registration instead of a bespoke `try`/`finally`.
- 2025-12-01 23:20 UTC: `UserDataMethodsTUnitTests.InteropTestAutoregisterPolicy` now relies on `UserDataRegistrationPolicyScope` + `UserDataRegistrationScope`, removing the final manual policy restore.
- 2025-12-01 23:25 UTC: `ProxyObjectsTUnitTests.ProxySurfaceAllowsAccessToRandom` now uses `UserDataRegistrationScope.Track<Proxy>` rather than manually unregistering the proxy type.
- 2025-12-01 23:30 UTC: `SimpleTUnitTests.SimpleDelegateInterop2` now wraps the automatic policy switch with `UserDataRegistrationPolicyScope`, eliminating another `try`/`finally`.
- 2025-12-01 23:35 UTC: `StructAssignmentTechniqueTUnitTests.StructFieldCantSetThroughLua` now uses `UserDataRegistrationScope` for the transform/vector registrations rather than a bespoke `try`/`finally`.
- 2025-12-01 23:45 UTC: Both `CollectionsBaseGenRegistered` and `CollectionsBaseInterfaceRegistered` helpers now guard their bulk registrations with `UserDataRegistrationScope`, dropping the large `try`/`finally` blocks.
- 2025-12-01 23:55 UTC: Added `ScriptToClrConversions.DynValueToObjectOfType<T>` plus tests and switched the converter specs over to the generic helper so coverage no longer relies on the reflection-based overload.
- Ease-of-use backlog: add helper scopes/wrappers for (a) `DynValue.ToObject<T>(...)` conveniences, (b) `UserDataRegistrationScope.Track` overloads (`params`/fluent `Register<T>` helpers), (c) `CustomConverterScope` for `Script.GlobalOptions.CustomConverters`, (d) generalized static-field override scopes for CLI/Debugger hooks, and (e) a shared `PlatformDetectorScope` for `PlatformDetectorIsolationAttribute`.
- 2025-12-02 00:05 UTC: Introduced `PlatformDetectorIsolationScope` and rewired `PlatformDetectorIsolationAttribute` to use it so platform detector tests now rely on the shared scope instead of bespoke semaphore/try-finally logic.
- 2025-12-02 00:10 UTC: Applied `UserDataRegistrationScope` to the `UserDataIsolationSmoke` tests so their isolation checks no longer hand-write register/unregister logic.
- 2025-12-02 00:15 UTC: Added `ProcessorCoroutineStackScope` and updated `ProcessorDebuggerRuntime` tests to use it, removing another manual coroutine stack restore.
- 2025-12-02 00:20 UTC: Added `ProcessorYieldScope` and applied it to `ProcessorCoroutineLifecycle` tests so coroutine yield flag overrides now flow through a shared disposable helper.
- Ease-of-use backlog: add helper scopes/wrappers for (a) `DynValue.ToObject<T>(...)` conveniences, (b) `UserDataRegistrationScope.Track` overloads (`params`/fluent `Register<T>` helpers), (c) `CustomConverterScope` for `Script.GlobalOptions.CustomConverters`, (d) generalized static-field override scopes for CLI/Debugger hooks, and (e) a shared `PlatformDetectorScope` for `PlatformDetectorIsolationAttribute`.
- 2025-12-01 23:50 UTC: `ScriptToClrConversionsTUnitTests.DynValueToObjectOfTypeHonorsCustomConverters` now wraps its converter taps with `Script.BeginGlobalOptionsScope` instead of a manual try/finally.
- Next steps: (a) finish adding helper scopes wherever ad-hoc cleanup still exists (remote-debugger harnesses/utilities are the remaining focus), (b) land the analyzer/CI rule that flags new detector/semaphore regressions so the temporary `rg`-based scripts can retire, and (c) continue the `ConfigureAwait(false)` sweep across the CLI suites so we can drop the outstanding CA2007 suppressions.
