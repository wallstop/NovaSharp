...
- Latest checkpoint: the CLI/Harness suites (`CliIntegrationTUnitTests`, `ProgramTUnitTests`, `RunCommandTUnitTests`, `HelpCommandTUnitTests`, `RegisterCommandTUnitTests`, `ShellCommandManagerTUnitTests`, `CompileCommandTUnitTests`, `HardwireCommandTUnitTests`, `DebugCommandTUnitTests`, `ExitCommandTUnitTests`) now wrap console capture via `ConsoleCaptureCoordinator.RunAsync`, so there are zero direct `SemaphoreSlim` waits/releases left under `src/tests/NovaSharp.Interpreter.Tests.TUnit/Cli/`.
- 2025-12-01 03:25 UTC: Added `TempDirectoryScope`/`TempFileScope` under `src/tests/TestInfrastructure/Scopes` and refactored representative suites (`FileSystemScriptLoaderTUnitTests`, `RunCommandTUnitTests`, `DotNetCorePlatformAccessorTUnitTests`) to consume the helpers, eliminating their temp-file/dir `try`/`finally` blocks. Interpreter and remote-debugger TUnit csproj files now link the shared scopes so the helpers are available repo-wide.
- 2025-12-01 03:30 UTC: Adopted the new scopes in `ProgramTUnitTests` (mod/run manifest helpers, hardwire output files) and `RemoteDebuggerServiceTUnitTests`, then removed the bespoke cleanup helpers from those suites. Remaining CLI suites still trigger CA2007 warnings that predate this work, so `dotnet build src/tests/NovaSharp.Interpreter.Tests.TUnit` currently fails until those warning suppressions or await patterns are restored.
- 2025-12-01 03:36 UTC: Converted `CliIntegrationTUnitTests`, `HardwireCommandTUnitTests`, `CompileCommandTUnitTests`, `ScriptRunTUnitTests`, `ScriptLoadTUnitTests`, and `ModManifestCompatibilityTUnitTests` to `TempDirectoryScope`/`TempFileScope`, deleting the last ad-hoc `Cleanup` helpers in those areas. The interpreter TUnit build still fails solely because of long-standing CA2007 complaints across the CLI suites; reintroduce the missing suppressions or add `ConfigureAwait` before treating the temp-scope work as blocked.
- 2025-12-01 03:45 UTC: Added `HardwireDumpLoaderScope` under `TestInfrastructure/Scopes` and refactored every test that previously captured/restored `HardwireCommand.DumpLoader` (Program, HardwireCommand, CliIntegration TUnit suites) to use `using` statements instead of manual `try`/`finally`. `dotnet build src/tests/NovaSharp.Interpreter.Tests.TUnit` still fails because of the pre-existing CA2007 warnings; once those warning suppressions land, rerun the build to validate the new scope coverage.
- Next steps: (a) add scopes for the remaining platform overrides (e.g., `PlatformDetectionTestHelper` still manually toggles Unity detection), (b) continue codemodding the remaining temp-resource cleanup paths (RemoteDebugger suites still rely on `CreateTempDirectory`/`TryDeleteDirectory`), and (c) wire a lightweight CI check that rejects new `ConsoleCaptureCoordinator.Semaphore` usages or trivial cleanup-only `try`/`finally` blocks to prevent regressions.
