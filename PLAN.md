...
- Progress snapshot (2025-11-29):
  - Microsoft.Testing.Platform remains the standard runner across the solution, and `src/tests/TestInfrastructure` now shares the same isolation helpers for NUnit and TUnit. The interpreter TUnit host executes **799** tests in ~2 s (`dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build`), covering VM/core suites, CLI helpers, loaders, modules, stream userdata, binary dump/varargs, metatables, collections, JSON serialization, closures, and the broader user-data field/property scenarios.
  - The remote-debugger TUnit project (`dotnet test --project src/tests/NovaSharp.RemoteDebugger.Tests.TUnit/NovaSharp.RemoteDebugger.Tests.TUnit.csproj -c Release`) exercises 35 scenarios in <0.9 s, including service/options coverage, jump-page flows, RPC port allocation, and attach-from-directory behavior so we can evaluate NUnit parity ahead of the final migration.
  - Platform detector and Io/RunFile convenience tests now run cleanly under TUnit after hardening the AOT cache, forcing the file-system loader when needed, and eliminating flakiness caused by shared temp paths.
  - 2025-11-29: Added `EndToEnd/UserDataMethodsTUnitTests` and ported the entire NUnit fixture to async TUnit helpers (registration scopes, ref/out semantics, constructor coverage, static vs. instance entry points, delegate binding). A focused TUnit run is blocked today by pre-existing failures in `CollectionsRegisteredTUnitTests` and the TUnit user-data field/property fixtures (null `Table.Set` arguments and CLR conversion errors), so document those failures when re-running the suite; see `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (2025-11-29) for the current stack traces.
  - 2025-11-29: Ported `EndToEnd/UserDataOverloadsTests` to `EndToEnd/UserDataOverloadsTUnitTests` (async helpers, extension-method coverage, static vs. instance overload validation, cache regressions) and verified compilation via `dotnet build src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`. A full `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` run still fails in the existing `CollectionsRegisteredTUnitTests`, `UserDataFieldsTUnitTests`, `UserDataPropertiesTUnitTests`, and `Descriptors/MethodMemberDescriptorTUnitTests` suites (same `ScriptRuntimeException`, `ArgumentNullException`, and multi-dimensional array issues listed above) before the new overload coverage can complete.
  - 2025-11-29: Ported `EndToEnd/UserDataEventsTests` to `EndToEnd/UserDataEventsTUnitTests` (async event handler wiring, static vs. instance events, detach/reregister paths) and confirmed the TUnit project still builds cleanly. Full `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` runs currently fail before we reach the new fixture because of the same collections/user-data descriptor issues, and when the event tests do execute `InteropSEventDetachAndReregister` still triggers the legacy “Cannot enter the same NovaSharp processor from two different threads” exception — track that failure alongside the other outstanding suites.
  - 2025-11-29: Ported `EndToEnd/UserDataIndexerTests` to `EndToEnd/UserDataIndexerTUnitTests` (multi-parameter indexers, metatable forwarding, metamethod guards, expression-list regression checks) and validated via `dotnet build`. Full interpreter `dotnet test` runs continue to fail in the same suites listed above, so this fixture currently compiles but cannot execute until those blockers are resolved.
  - 2025-11-29: Ported `EndToEnd/UserDataNestedTypesTests` to `EndToEnd/UserDataNestedTypesTUnitTests` (enum exposure, nested reference/value types, NovaSharpUserData private types, failure paths) and confirmed `dotnet build` stays clean. The interpreter TUnit test run still fails early in the known collections/user-data descriptor suites, so this fixture is build-only for now.
  - 2025-11-29: Ported `EndToEnd/UserDataEnumsTests` to `EndToEnd/UserDataEnumsTUnitTests` (enum overload resolution, flags helpers, camel-case matching, default property access) and verified the project builds. Interpreter-wide `dotnet test` remains blocked by the same pre-existing suites (`CollectionsRegisteredTUnitTests`, `UserDataFields/PropertiesTUnitTests`, `Descriptors/MethodMemberDescriptorTUnitTests`), so enum coverage currently compiles but cannot run yet.
  - 2025-11-29: Ported `EndToEnd/UserDataMetaTests` to `EndToEnd/UserDataMetaTUnitTests` (metamethod iteration pairs/ipairs, length/count fallbacks, equality/comparison operators, arithmetic/concat/pow/call). Build stays green, but full interpreter `dotnet test` attempts still halt in the known collections/user-data descriptor suites, so the meta coverage is build-only at the moment.
  - 2025-11-29: Ported the NUnit `Units/UserDataIsolationTests` to `Units/UserDataIsolationTUnitTests` so the shared isolation helpers (registration policy, access mode, concurrent scope behavior) now run under the Microsoft.Testing.Platform host. `dotnet test` still fails earlier in the same suites listed above, so this unit-focused coverage currently only compiles.
  - 2025-11-29: Ported the NUnit `Units/UserDataTests` to `Units/UserDataTUnitTests` (proxy registration, extension methods, descriptor resolution, helper registries, exception cases). Build stays clean, but the interpreter-wide `dotnet test` command remains blocked by the pre-existing collections/user-data descriptor suites and the event-thread issue, so this unit suite hasn’t executed yet.
