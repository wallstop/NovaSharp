...
- Tests: `scripts/coverage/coverage.ps1` now drives every suite via Microsoft.Testing.Platform. The latest run (2025-11-30 14:20 UTC) executed **2,373** interpreter/remote TUnit tests (`src/tests/NovaSharp.Interpreter.Tests.TUnit`) with zero failures while `FixtureCatalogGenerated.cs` tracks **31** remaining NUnit fixtures (down from 75 at the start of the initiative).
...
- **Progress snapshot (2025-11-30):** The interpreter TUnit suite now runs **2,373** tests in ~6 s (latest run at 14:20 UTC) with `FixtureCatalogGenerated.cs` tracking **31** NUnit fixtures. The Hardwire descriptor stack (code generation context, descriptor/member/method/userdata fixtures), DebugModule, LoadModule, LuaBase, HardwireParameter, CLI/REPL, os, and IO suites all live on TUnit, and `rg -l "[TestFixture]" src/tests/NovaSharp.Interpreter.Tests | Measure-Object` reports the remaining fixtures enumerated below.
- **2025-11-30 14:20 UTC checkpoint:** Migrated `Units/StandardIoFileUserDataBaseTests` to `Modules/StandardIoFileUserDataBaseTUnitTests`, regenerated `FixtureCatalogGenerated.cs` (now **43** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (suite now **2,373** tests passing).
- **2025-11-30 22:30 UTC checkpoint:** Migrated `Units/ClosureContextTests` to `Units/ClosureContextTUnitTests`, deleted the NUnit fixture, regenerated `FixtureCatalogGenerated.cs` (now **42** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (still **2,373** tests passing in ~6.5 s). The new TUnit assertions also verify the constructor argument guard so coverage stays at parity.
- **2025-11-30 22:33 UTC checkpoint:** Migrated `Units/DynamicExprExpressionTests` and `Units/AdjustmentExpressionTests` to `Units/DynamicExprExpressionTUnitTests` and `Units/AdjustmentExpressionTUnitTests`, removed the shared NUnit fixture/helper, regenerated `FixtureCatalogGenerated.cs` (now **40** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (suite remains **2,373** tests passing in ~6 s). The TUnit port keeps the stub expression helper internal and preserves the bytecode/tuple coverage so the expression suite stays green.
- **2025-11-30 22:35 UTC checkpoint:** Migrated `Units/InfrastructureTests` to `Units/InfrastructureTUnitTests`, deleted the NUnit fixture, regenerated `FixtureCatalogGenerated.cs` (now **39** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (suite still **2,373** tests in ~6 s). The TUnit coverage now exercises `PerformanceResult`, `DynamicExpressionException`, `InternalErrorException`, and `ExecutionState` without relying on NUnit adapters.
- **2025-11-30 22:37 UTC checkpoint:** Migrated `Units/CloseAttributeTests` to `Units/CloseAttributeTUnitTests`, removed the NUnit file, regenerated `FixtureCatalogGenerated.cs` (now **38** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (still **2,373** tests in ~6 s). The new TUnit coverage keeps the Lua `<close>` regression scripts, including error propagation and loop/goto scope exits, so resource semantics continue to match Lua 5.4 expectations.
- **2025-11-30 22:38 UTC checkpoint:** Removed `Units/StreamFileUserDataBaseTests` after confirming its TUnit counterpart (`Modules/StreamFileUserDataBaseTUnitTests`) carries the coverage, regenerated `FixtureCatalogGenerated.cs` (now **37** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (suite still **2,373** tests). This drops one of the heaviest NUnit fixtures from the interpreter project without losing IO/file semantics coverage.
- **2025-11-30 22:39 UTC checkpoint:** Removed `Units/SymbolRefTests` because `Units/SymbolRefTUnitTests` already provides identical serialization/flag coverage, regenerated `FixtureCatalogGenerated.cs` (now **36** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (suite still **2,373** tests). The interpreter NUnit surface continues to shrink without sacrificing SymbolRef behavior validation.
- **2025-11-30 22:40 UTC checkpoint:** Removed `Units/JsonNullTests` since `Units/JsonNullTUnitTests` already covers the JsonNull helpers, regenerated `FixtureCatalogGenerated.cs` (now **35** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (suite still **2,373** tests). This keeps the interpreter NUnit list focused on fixtures that still need dedicated ports rather than duplicates.
- **2025-11-30 22:42 UTC checkpoint:** Migrated `Units/ClosureTests` to `Units/ClosureTUnitTests`, deleted the NUnit fixture, regenerated `FixtureCatalogGenerated.cs` (now **33** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (suite still **2,373** tests in ~6 s). Closure internals are now fully covered by TUnit.
- **2025-11-30 22:43 UTC checkpoint:** Removed `Units/JsonTableConverterTests` because `SerializationTests/Json/JsonTableConverterTUnitTests` already exercises the same scenarios, regenerated `FixtureCatalogGenerated.cs` (now **33** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (suite still **2,373** tests). The interpreter NUnit set is now limited to fixtures without TUnit equivalents.
- **2025-11-30 22:45 UTC checkpoint:** Removed `Units/OsSystemModuleTests` since `Modules/OsSystemModuleTUnitTests` already carries the module coverage, regenerated `FixtureCatalogGenerated.cs` (now **32** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (suite still **2,373** tests).
- **2025-11-30 22:46 UTC checkpoint:** Migrated `Units/CompatibilityDiagnosticsTests` to `Units/CompatibilityDiagnosticsTUnitTests`, deleted the NUnit fixture, regenerated `FixtureCatalogGenerated.cs` (now **31** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` (suite still **2,373** tests).
  - **Outstanding interpreter fixtures:** Focus next on the VM-heavy NUnit suites that still exist (`ProcessorTests`, `CoroutineApiTests`, `CoroutineCloseTests`, lingering Script/Debugger helpers, and the remaining module/tooling adapters). Clearing those high-risk files will collapse the remaining **31** NUnit fixtures quickly so we can retire the interpreter NUnit project.
