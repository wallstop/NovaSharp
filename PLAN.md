...
- Progress to date:
  - Microsoft.Testing.Platform is enforced repo-wide; NUnit fixtures run via the consolidated `scripts/tests/NovaSharp.Parallel.runsettings`, and isolation attributes were refactored into `src/tests/TestInfrastructure/NUnit`.
  - The TUnit host (`NovaSharp.Interpreter.Tests.TUnit`) is live with shared TestUtilities, and compare harness artefacts live under `artifacts/tunit-migration/*.json`.
  - Completed TUnit suites: remote-debugger pilot, ScriptLoad, ScriptRun, ScriptCall, ScriptExecutionContext, ScriptLoaderBase, ScriptDefaultOptions, ScriptPrivateResourceExtension, ScriptTypeMetatable, ScriptLoadFile, ScriptRuntimeException, ScriptToClrConversions, ClrToScriptConversions, ProcessorStackTrace, ProcessorBinaryDump, CoroutineLifecycle (artefacts kept in the migration folder). These no longer need explicit checkpoints in PLAN.md.
...
- **Next steps (2025-11-28 update):**
  1. Continue the VM/core migration by targeting the remaining `ProcessorTests` sections (close-symbol/block cleanup, iterator/table opcodes, bit ops, index handling) before moving on to `ProcessorDebuggerTests`; capture compare artefacts for each batch so the migration cadence stays documented.
  2. As soon as the VM/core set is complete, start planning the Processor/Coroutine fixtures (identify blockers, required helpers) so Phase‑1 can close on schedule.
