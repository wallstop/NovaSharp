...
- 2025-11-27: Repl loader tests no longer mutate real environment variables. `ReplInterpreterScriptLoader` now accepts a custom environment provider, and the fixture uses that hook to emulate `NOVASHARP_PATH`/`LUA_PATH_*` scoped per test, so `[Parallelizable(ParallelScope.Self)]` is enabled for `ReplInterpreterScriptLoaderTests`. With IO/REPL suites isolated we immediately wired `dotnet test` (locally and in CI) to the shared runsettings profile described below so multi-worker execution stays enabled by default.
- 2025-11-27: Added `PlatformDetectorIsolationAttribute` to serialize access to `PlatformAutoDetector` state, then switched `PlatformAutoDetectorTests` and `PlatformAccessorBaseTests` to `[Parallelizable(ParallelScope.Self)]`. Verified both fixtures with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --no-build --settings scripts/tests/NovaSharp.Parallel.runsettings --filter FullyQualifiedName~PlatformAutoDetectorTests` and `--filter FullyQualifiedName~PlatformAccessorBaseTests`.
- 2025-11-27: Enabled `[Parallelizable(ParallelScope.Self)]` for `IoModuleTests` (57 Release tests) and `IoModuleVirtualizationTests` by decorating each fixture with `[ScriptGlobalOptionsIsolation]` so their `Script.GlobalOptions.Platform` overrides stay test-local. Verified via `dotnet test --project src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --no-build --settings scripts/tests/NovaSharp.Parallel.runsettings --filter FullyQualifiedName~IoModuleTests` and `--filter FullyQualifiedName~IoModuleVirtualizationTests`. IO suites now run in parallel with the rest of the interpreter tests.
- 2025-11-27: Extended `[ScriptGlobalOptionsIsolation]` + `[Parallelizable(ParallelScope.Self)]` to the remaining compatibility/descriptor/exception fixtures (`CompatibilityVersionTests`, `ModManifestTests`, `FieldMemberDescriptorTests`, `MethodMemberDescriptorTests`, `PropertyMemberDescriptorTests`, `PropertyTableAssignerTests`, `ScriptRuntimeExceptionTests`, `SyntaxErrorExceptionTests`). Verified each fixture with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --no-build --settings scripts/tests/NovaSharp.Parallel.runsettings --filter FullyQualifiedName~CompatibilityVersionTests` (and the corresponding filters for the other seven fixtures) so every `Script.GlobalOptions` consumer now runs safely in parallel.
