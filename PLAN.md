...
- 2025-11-26 20:55 UTC: Retired the module-level CA1515 suppression in `NovaSharp.Comparison` by making `ScriptScenario` and `LuaPerformanceBenchmarks` internal (BenchmarkDotNet discovers them via `BenchmarkSwitcher` without requiring public visibility) and deleting the `GlobalSuppressions.cs`. Rebuilt the comparison project (`dotnet build src/tooling/NovaSharp.Comparison/NovaSharp.Comparison.csproj -c Release`) and re-ran the documentation/naming/spelling audits to lock in the analyzer baseline.
- 2025-11-26 21:20 UTC: Removed the remaining CA1515 suppression from `NovaSharp.Benchmarks` by adding `InternalsVisibleTo("BenchmarkDotNet.Autogenerated")`, making the benchmark classes/enums (`ScriptLoadingBenchmarks`, `RuntimeBenchmarks`, `BenchmarkHost`, `ScriptComplexity`, `RuntimeScenario`) internal, and deleting `GlobalSuppressions.cs`. Verified with `dotnet build src/tooling/Benchmarks/NovaSharp.Benchmarks/NovaSharp.Benchmarks.csproj -c Release` plus the doc/naming/spelling audits so the analyzer gate stays clean.
