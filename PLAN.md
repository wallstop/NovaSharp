...
- Progress snapshot (2025-11-29):
  ...
  - 2025-11-29 (latest run): `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` now reports **71** failures. The fallout is concentrated in `EndToEnd/CollectionsRegisteredTUnitTests` (user-data conversions + multi-dimensional array handling), the user-data field/property suites (`ArgumentNullException` when pushing wrapped objects into `Globals`), and the descriptor/unit coverage (`RegisterTypeKeepsExistingDescriptor…`, `CustomDescriptorIsUsed…`, etc.). `Isolation/UserDataIsolationSmokeTUnitTests` continues to show leaked registrations, so the next milestone is (1) validate that the start/end receivers actually dispose the scope per test, (2) keep the collections fixture aligned with the NUnit baseline (`script.Globals["o"] = host` vs. `UserData.Create`), and (3) once isolation is reliable, resume chipping away at the user-data field/property conversions that currently hit `Table.Set` null checks.
  - 2025-11-29 (late): Swapped the isolation attributes back to explicit `ITestStartEventReceiver`/`ITestEndEventReceiver` implementations that stash their scope in `TestContext.StateBag` so the begin/dispose calls happen directly on the Microsoft.Testing.Platform worker thread (no more `ScopedHookExecutor` indirection). This keeps the plumbing simple while we continue chasing the remaining suite failures in `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build`.
  - 2025-11-29 (latest run): `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` now reports **71** failures. The fallout is still concentrated in `EndToEnd/CollectionsRegisteredTUnitTests` (user-data conversions + multi-dimensional array handling), the user-data field/property suites (`ArgumentNullException` when pushing wrapped objects into `Globals`), and the descriptor/unit coverage (`RegisterTypeKeepsExistingDescriptor…`, `CustomDescriptorIsUsed…`, etc.). `Isolation/UserDataIsolationSmokeTUnitTests` continues to show leaked registrations, so the next milestone is (1) validate that the reintroduced start/end receivers actually dispose the scope per test, (2) keep the collections fixture aligned with the NUnit baseline (`script.Globals["o"] = host` vs. `UserData.Create`), and (3) once isolation is reliable, resume chipping away at the user-data field/property conversions that currently hit `Table.Set` null checks.
  - 2025-11-29 (late): Retried the isolation hook by switching `UserDataIsolationAttribute`/`ScriptGlobalOptionsIsolationAttribute` to `EventReceiverStage.Late`; this keeps  `BeginIsolationScope()` attached to the Microsoft.Testing.Platform worker thread more often, trimming the interpreter TUnit failure count from 93 → 82 (`dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build`). The remaining failures are still concentrated in `CollectionsRegisteredTUnitTests`, the UserData field/property suites, the enum overloads, and the descriptor/unit fixtures, so we still need a deterministic execution hook (or custom `HookExecutor` shim) that wraps the actual test body before these suites can run green.
  - 2025-11-29 (even later): Replaced the direct `ITestStartEventReceiver` scope creation with explicit event registrations via `context.Events.OnTestStart`/`OnTestEnd`, so the user-data/global-options scopes now begin inside the Microsoft.Testing.Platform worker that actually executes the test body. Added `Isolation/UserDataIsolationSmokeTUnitTests` to prove registrations never leak between tests when `[UserDataIsolation]` is present. The interpreter TUnit run still reports 76 failures (`dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build`), concentrated in the CollectionsRegistered, UserData field/property/enum/event, and descriptor/unit suites, so the remaining work is real fixture fallout rather than missing infrastructure.
