...
- 2025-11-28 checkpoint: exercised the legacy `IScriptLoader` reflection path via `ScriptLoadTests.LoadFileInvokesLegacyResolveFileNameFallback`, ensuring `ResolveFileNameWithLegacyFallback` stays covered for non-`ScriptLoaderBase` loaders.
...
- 2025-11-28 checkpoint: added `ScriptLoaderBaseTests` so `UnpackStringPaths` and the protected `ResolveModuleName(..., IEnumerable<string>)` path stay covered, tightening loader behavior before we re-enable interpreter branch gating.
- 2025-11-28 checkpoint: added `CoroutineTests.AutoYieldCounterForcesSuspendUntilResumed` so the forced-yield branch (AutoYieldCounter) on `Coroutine.Resume` is now covered, shaving down the coroutine branch debt.
...
- 2025-11-28 checkpoint: covered the UnityAssetsScriptLoader default-path branch via `UnityAssetsScriptLoaderTests.DefaultConstructorUsesDefaultPath`, ensuring the reflection loader requests `NovaSharp/Scripts` when no custom path is supplied.
