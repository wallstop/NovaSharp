...
- **2025-11-30 09:28 UTC update:** Linked the shared Hardwire/TestUtilities helpers into the TUnit project, added `HardwireParameterDescriptorTUnitTests`, fixed the analyzer violations (StringComparison, CA1806, CoreModules.None usage) in the newly ported suites, deleted the redundant NUnit fixtures, regenerated `FixtureCatalogGenerated.cs`, and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release --no-build` to validate the expanded suite (total TUnit tests: **2,136**).
 ...
 - **2025-11-30 17:55 UTC checkpoint:** Removed the legacy NUnit `EndToEnd/CoroutineTests` fixture now that `EndToEnd/CoroutineTUnitTests` covers the same scenarios, then regenerated `FixtureCatalogGenerated.cs` (count now **74** fixtures). This keeps NUnit/TUnit parity tidy and moves the interpreter suite another step toward full TUnit adoption.
 - **2025-11-30 17:57 UTC checkpoint:** Ported the NUnit `EndToEnd/TailCallTests` cases into `TailCallTUnitTests` (recursive sum stress, CLR tail-call propagation, and `tostring` metamethod assertions), deleted the NUnit file, regenerated `FixtureCatalogGenerated.cs` (now **73** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` to validate the expanded suite (**2,141** tests passed).
 - **2025-11-30 18:00 UTC checkpoint:** Deleted the redundant NUnit end-to-end fixtures for BinaryDump, Closure, CollectionsRegistered, ErrorHandling, and VarargsTuple now that their TUnit counterparts exist; regenerated `FixtureCatalogGenerated.cs` (down to **68** fixtures) and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` to confirm the TUnit suite (**2,141** tests) still passes.
  - **2025-11-30 18:03 UTC checkpoint:** Added `EndToEnd/ConfigPropertyAssignerTUnitTests`, removed the NUnit fixture, regenerated `FixtureCatalogGenerated.cs` (now **67** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` to verify the expanded TUnit run (**2,143** tests succeeded).
- **2025-11-30 18:05 UTC checkpoint:** Removed the remaining duplicate NUnit fixtures for JsonSerialization, Metatable, RealWorldScript, StringLib, and Table now that their TUnit suites are authoritative; `FixtureCatalogGenerated.cs` is down to **62** fixtures and `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` continues to pass (**2,143** tests).
- **2025-11-30 18:08 UTC checkpoint:** Ported the NUnit `DynamicTests`, `GotoTests`, and `StructAssignmentTechnique` fixtures into dedicated TUnit suites, deleted the old files, regenerated `FixtureCatalogGenerated.cs` (now **59** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (now **2,159** tests passing).
- **2025-11-30 18:48 UTC checkpoint:** Added TUnit coverage for proxy interop and fully ported the giant `SimpleTests` fixture (now `SimpleTUnitTests`), deleted the corresponding NUnit files plus the legacy `EndToEnd/Utils` helper, pulled in the minimal NUnit assertion package for the migrated suite, regenerated `FixtureCatalogGenerated.cs` (now **57** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (now **2,242** tests passing).
- **2025-11-30 19:05 UTC checkpoint:** Migrated the parser/token/syntax-error unit fixtures to TUnit (`ParserTUnitTests`, `TokenTUnitTests`, `SyntaxErrorExceptionTUnitTests`), deleted the NUnit originals, regenerated `FixtureCatalogGenerated.cs` (now **54** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (suite expanded to **2,271** tests).
- **2025-11-30 20:39 UTC checkpoint:** Ported the remaining lexical helpers (`LexerUtilsTests`, `LiteralExpressionTests`, `ExprListExpressionTests`, `SymbolRefTests`) into TUnit, removed the NUnit files, regenerated `FixtureCatalogGenerated.cs` (now **51** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (suite now **2,304** tests).
- **2025-11-30 20:41 UTC checkpoint:** Added module/platform coverage to TUnit (`DynamicModuleTUnitTests`, `StandardPlatformAccessorTUnitTests`), deleted the remaining NUnit fixtures for those areas, regenerated `FixtureCatalogGenerated.cs` (now **49** fixtures), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (suite now **2,324** tests).
