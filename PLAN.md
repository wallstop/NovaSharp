...
- 2025-12-02 01:34 UTC: CA2007 is now enforced via `.editorconfig`, all suppressions were removed, and `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` stays green with the analyzer gate enabled.
- 2025-12-02 02:25 UTC: Remote-debugger adapter coverage now includes both the VS Code server (`VsCodeDebugServerTUnitTests`) and the ProcessLoop-driven harness (`VsCodeDebugSessionTUnitTests`). These suites verify attach/detach semantics, REPL commands (`!list`, `!seterror`, `!switch`), breakpoints, and queued actions while keeping `dotnet test src/tests/NovaSharp.RemoteDebugger.Tests.TUnit/NovaSharp.RemoteDebugger.Tests.TUnit.csproj -c Release` green.
- 2025-12-02 02:32 UTC: CLI automation now covers the `debug`, `run`, and `register` commands with dedicated TUnit suites that assert syntax help, compatibility logging, debugger attachment, and userdata registration flows. `dotnet test src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` remains green (2,726 tests, ~7.4 s).
- 2025-12-01 22:20 UTC: Replaced the remaining manual `try`/`finally` cleanup in `Cli/DebugCommandTUnitTests.cs`, `Units/HardwireGeneratorTUnitTests.cs`, and `TestInfrastructure/ConsoleCaptureCoordinator.cs` with `DebugCommandScope`, `SemaphoreSlimScope`, and `ConfiguredAsyncDisposable`-backed leases. `rg -n 'finally' src/tests` now returns zero matches, and `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (2,726 tests) passes with CA2007 enforced.
- 2025-12-01 22:27 UTC: Expanded `UserDataRegistrationScope` with multi-type `Track` overloads plus `RegisterType` helpers, migrated the CLI RegisterCommand coverage, and converted both collections-registered suites (`CollectionsBaseInterfaceRegisteredTUnitTests`, `CollectionsBaseGenRegisteredTUnitTests`) to those helpers. Userdata-heavy fixtures now register/cleanup atomically without manual `UserData.RegisterType` lists, and the interpreter TUnit run stays green.
- 2025-12-01 22:33 UTC: `EndToEnd/UserDataEventsTUnitTests` now uses a dedicated `WithRegisteredEventTypes` helper that scopes the `SomeClass`/`EventArgs` registrations via `UserDataRegistrationScope` (including the static-event variants). This removed seven more manual registration sites while keeping `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` green with CA2007 enforced.
- 2025-12-01 22:36 UTC: `EndToEnd/UserDataNestedTypesTUnitTests` now wraps every scenario in `WithRegisteredTypeAsync`, so both `SomeType` and `VSomeType` registrations flow through `UserDataRegistrationScope` and clean up automatically. The suite no longer calls `UserData.RegisterType` directly, aligning it with the test cleanup helper initiative.
- 2025-12-01 22:40 UTC: `EndToEnd/UserDataPropertiesTUnitTests` now registers `SomeClass` exclusively through `UserDataRegistrationScope` (via a new `Action<UserDataRegistrationScope>`-based `WithRegistrationAsync` helper). Every getter/setter test passes the desired `InteropAccessMode` into the scope instead of calling `UserData.RegisterType` directly, eliminating 30+ manual registrations while keeping `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` green.
- 2025-12-01 22:43 UTC: Mirrored the same helper pattern in `EndToEnd/UserDataFieldsTUnitTests`, so all field getter/setter tests now call `scope.RegisterType<SomeClass>(mode)` instead of `UserData.RegisterType`. This eliminates another large block of manual registrations while preserving the existing assertions.
- 2025-12-01 22:46 UTC: `EndToEnd/UserDataMethodsTUnitTests` now acquires a `UserDataRegistrationScope` whenever `SomeClass` is needed (including the static/constructor/varargs helpers and the camel-case regression tests). All manual `UserData.UnregisterType<SomeClass>()`/`RegisterType<SomeClass>(...)` pairs have been replaced with `using` scopes via the new `RegisterSomeClass` helper, keeping the interpreter TUnit suite green.
- 2025-12-01 22:49 UTC: `EndToEnd/UserDataEnumsTUnitTests` registers the host/enum types by creating a scoped `UserDataRegistrationScope` before each overload test. The helper now handles `EnumOverloadsTestClass`, `SampleRating`, and `SampleFlagSet` registrations instead of calling `UserData.RegisterType` directly.
- 2025-12-01 22:51 UTC: `EndToEnd/UserDataMetaTUnitTests` and `EndToEnd/UserDataOverloadsTUnitTests` now rely on `UserDataRegistrationScope` (plus existing extension-type registrations) instead of manual `UserData.RegisterType` calls, removing the remaining direct registrations for the meta/overload suites.
- Outstanding suites still calling `UserData.RegisterType(...)` include the descriptor regression tests (`DispatchingUserDataDescriptorTUnitTests`, `MethodMemberDescriptorTUnitTests`, `HardwiredDescriptorTUnitTests`, `StandardGenericsUserDataDescriptorTUnitTests`), VM/module helpers (`ClrToScriptConversionsTUnitTests`, `StreamFileUserDataBaseTUnitTests`, `DebugModuleTUnitTests`, `ColonOperatorBehaviourTUnitTests`, `DynValueTUnitTests`, `ValueTypeDefaultCtorMemberDescriptorTUnitTests`, `DynamicModuleTUnitTests`), CLI smoke suites (`UserDataIsolationSmokeTUnitTests`, `RegisterCommandTUnitTests`), and the infrastructure-heavy `UserDataTUnitTests`. **Next steps:** keep sweeping those areas to adopt `UserDataRegistrationScope`, add shared helpers where bulk registration is needed (descriptor catalogs, CLI fixtures), and continue reducing the `rg -n "UserData.RegisterType"` footprint until only intentional extension registrations remain.
