...
1. **Table storage is no longer the primary structural blocker**: A4 replaced the linked-list/index maps with an array part plus dense bucketed hash part. The remaining A1c table costs are heap `DynValue` elements/nodes and the temporary `_arrayKeys` wrapper memo; re-measure and remove the memo after values become inline structs.
...
See the [Strategic Roadmap](#-strategic-roadmap-performance-parity--unity-first-api-2026) at the top of this document. A0, B0, A1a, and A1b are complete. Immediate sequence: finish **A1c** (explicit absence protocols → VM-native readonly `LuaValue`) → **A1d** measured copy tuning → A4 scoreboard re-measure/memo cleanup → **A5** stack-window calls. Everything below is subordinate.
