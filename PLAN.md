...
- 2025-11-26 18:05 UTC: Removed the lingering CA1822 suppression in `src/tests/NovaSharp.Interpreter.Tests/Units/MethodMemberDescriptorTests.cs` by reinstating the missing `[Test]` attribute, adding `_compatibilityLock` to serialize `CheckMethodIsCompatible` mutations, and letting the analyzer enforce the rule without a waiver. `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter MethodMemberDescriptorTests` now exercises all 53 fixture cases, including the restored open-generic rejection scenario.
- 2025-11-26 18:45 UTC: Cleared the CA1823 suppressions by (a) instantiating `VisibilityFixtures.PrivateNested` inside a static constructor instead of via a fake anchor field, and (b) letting `UserDataFieldsTests.SomeClass` trim its `_privateProp` sentinel so the private field is genuinely read. Verified with `dotnet test ... --filter DescriptorHelpersTests` plus `... --filter UserDataFieldsTests`; both suites remain green while analyzers now enforce CA1823 automatically.
- 2025-11-26 19:30 UTC: Retired the CA1819 allowances by converting `TailCallData.Args` and `YieldRequest.ReturnValues` into `ReadOnlyMemory<DynValue>` views backed by shared arrays. Added span/borrow helpers for VM internals (`Processor`, `ScriptExecutionContext`, coroutine resume) and updated the NUnit suites (`ScriptExecutionContextTests`, `CoroutineModuleTests`) to assert against spans. Full interpreter regression (`dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release`) stays green, and the doc/naming/spelling audits now pass without CA1819 suppressions.
- 2025-11-26 20:05 UTC: Removed the CA1051 suppressions in `DescriptorHelpersTests` by switching the visibility fixtures to static fields (`PublicFieldValue`, `InternalFieldValue`, etc.) and binding the metadata via explicit `BindingFlags`. This keeps the field-visibility unit tests intact while complying with the “no visible instance fields” rule. Validated through `dotnet test ... --filter DescriptorHelpersTests` plus the documentation/naming/spelling audits.
- 2025-11-26 20:35 UTC: Cleared the remaining CA1812 suppressions by (a) instantiating the private Lua user-data fixtures (`SomeType` static ctor) and the protected visibility fixture type, and (b) touching `LuaPerformanceBenchmarks` once from `Program.Main` so the comparison assembly no longer relies solely on BenchmarkDotNet reflection. Confirmed via `dotnet test ... --filter UserDataNestedTypesTests`, `dotnet test ... --filter DescriptorHelpersTests`, and `dotnet build src/tooling/NovaSharp.Comparison/NovaSharp.Comparison.csproj -c Release`, with all audits refreshed afterward.
- 2025-11-26 20:55 UTC: Retired the module-level CA1515 suppression in `NovaSharp.Comparison` by deleting `GlobalSuppressions.cs`, keeping the benchmark surface public (required by BenchmarkDotNet), and adding targeted `[SuppressMessage]` annotations on `ScriptScenario` and `LuaPerformanceBenchmarks`. Rebuilt the comparison project (`dotnet build src/tooling/NovaSharp.Comparison/NovaSharp.Comparison.csproj -c Release`) and re-ran the documentation/naming/spelling audits to lock in the analyzer baseline.
- 2025-11-26 21:20 UTC: Removed the remaining CA1515 suppression from `NovaSharp.Benchmarks` by deleting `GlobalSuppressions.cs`, adding `[SuppressMessage]` annotations to the necessary public benchmark types/enums (`ScriptLoadingBenchmarks`, `RuntimeBenchmarks`, `ScriptComplexity`, `RuntimeScenario`), and granting `InternalsVisibleTo("BenchmarkDotNet.Autogenerated")` so future internal helpers can stay non-public. Verified with `dotnet build src/tooling/Benchmarks/NovaSharp.Benchmarks/NovaSharp.Benchmarks.csproj -c Release` plus the doc/naming/spelling audits so the analyzer gate stays clean.
...
- 2025-11-26 22:05 UTC: Added bitwise-not regression tests in `UnaryOperatorExpressionTests` to cover both the success and failure paths for the `~` unary operator, eliminating the remaining holes in that expression’s branch coverage. Verified with `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter UnaryOperatorExpressionTests` and refreshed the documentation/naming/spelling audits afterward.
- 2025-11-26 22:25 UTC: Extended `UnityAssetsScriptLoaderTests` with null-argument guards so both `LoadFile(null, …)` and `ScriptFileExists(null)` paths throw `ArgumentNullException`, covering the remaining guard branches in `UnityAssetsScriptLoader`. Validated with `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter UnityAssetsScriptLoaderTests` and reran the documentation/naming/spelling audits.
- 2025-11-26 22:45 UTC: Added coroutine regression tests for `AsUnityCoroutine` (ensuring every iteration yields `null` and transitions to the dead state) plus the `Resume(context, params object[])` overload, closing untested branches in `Coroutine.AsUnityCoroutine` and the context/object resume path. Verified with `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter CoroutineTests` and refreshed the audit logs so coverage tracking stays accurate.
- 2025-11-26 23:05 UTC: Boosted `PlatformAutoDetector` coverage by (a) letting the auto-detection path run without Unity assemblies (`GetDefaultScriptLoader` now hits the `unityTypeFound = false` loop) and (b) parameterising the AOT-probe exception test to include `SecurityException` alongside `PlatformNotSupportedException`. Validated with `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter PlatformAutoDetectorTests` and refreshed the documentation/naming/spelling audits.
- 2025-11-26 23:20 UTC: Exercised `Script.RequireModule` via a module-aware stub loader so both the success path (string-returning loader) and the missing-module exception are covered; the loader now records resolved module names and loaded files, ensuring the branch that forwards to `LoadFile` is driven under NUnit. Verified with `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter ScriptLoadTests` and reran the documentation/naming/spelling audits.
- 2025-11-26 23:35 UTC: Added null-argument regression tests for `Script.Dump` so the defensive guards (`function`/`stream`) execute under NUnit, further closing the Script-class branch gap. Validated with `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter ScriptLoadTests` and refreshed all audits.
- 2025-11-26 23:50 UTC: Added `LoadStreamUndumpPreservesEnvironmentUpValues`, which dumps a function that reads globals, loads it into a new `Script`, and verifies the restored closure reads from the new environment. This drives the `hasUpValues` branch inside `LoadStream` (the path that injects `_ENV` when undumping). Tests still pass via `dotnet test … --filter ScriptLoadTests`, and documentation/naming/spelling audits were rerun afterward.
- 2025-11-27 00:05 UTC: Expanded `Bit32CompatibilityWarningTests` so Lua 5.2 compatibility runs (which ship the bit32 library) call `require('bit32')` without triggering the warning and assert the loader still succeeds. This covers the “profile supports bit32” branch in `WarnIfBit32CompatibilityDisabled`. Validated with `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter Bit32CompatibilityWarningTests` plus the documentation/naming/spelling audits.
