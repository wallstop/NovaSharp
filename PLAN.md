...
- **Progress snapshot (2025-11-29):** The interpreter TUnit suite now runs **1,862** tests in ~6.9 s (per the latest coverage run) with `FixtureCatalogGenerated.cs` tracking **99** fixtures. BinaryOperatorExpression, DescriptorHelpers, DynValue, DynamicExpression, and FunctionCall fixtures have been ported to TUnit, while `rg -l "[TestFixture]" src/tests/NovaSharp.Interpreter.Tests | Measure-Object` still reports **98** NUnit fixtures that must be retired.
  - **Outstanding interpreter fixtures:** Focus next on the heaviest remaining NUnit suites—`IoModuleTests.cs`, `LuaStateInteropToolsTests.cs`, `MemberDescriptorTests.cs`, `ProcessorTests.cs`, and the remaining module/tooling helpers under `Units/`. Tackling these batches will collapse the residual 98 fixtures quickly so the interpreter NUnit project can be retired.
