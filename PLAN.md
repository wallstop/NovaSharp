...
- Build: `dotnet build src/NovaSharp.sln -c Release -nologo` (2025-11-29 17:18 UTC) finished with zero warnings while `<TreatWarningsAsErrors>true>` stays enforced across the solution.
- Tests: `scripts/coverage/coverage.ps1` now drives both suites via Microsoft.Testing.Platform. The latest run (2025-11-29 17:21 UTC) executed **1,862** TUnit tests (`src/tests/NovaSharp.Interpreter.Tests.TUnit`) and **1,309** NUnit tests (`src/tests/NovaSharp.Interpreter.Tests`) with zero skips/failures, keeping `FixtureCatalogGenerated.cs` in sync.
- Coverage: `docs/coverage/latest/Summary.md` (2025-11-29 17:21 UTC) reports **82.6 % line / 84.2 % branch / 84.5 % method** overall after the combined Release run (total **3,171** tests).
  - NovaSharp.Interpreter: 96.1 % line / 93.8 % branch (still <95 %, so `COVERAGE_GATING_MODE` remains in monitor mode).
  - NovaSharp.Cli: 81.7 % line / 76.3 % branch.
  - NovaSharp.Hardwire: 55.7 % line / 46.6 % branch.
  - NovaSharp.RemoteDebugger: 0.19 % line / 0 % branch (coverage slipped after the latest refactors; debugger automation remains blocked).
  - NovaSharp.VsCodeDebugger: 0 % line / 0 % branch (awaiting automated debugger smoke tests).
- Coverage collateral: rerun `./scripts/coverage/coverage.ps1` (now Microsoft.Testing.Platform-aware) and refresh `docs/coverage/coverage-hotspots.md` whenever numbers move; last refresh completed 2025-11-29 17:21 UTC with the figures above.
...
- **Progress snapshot (2025-11-29):** The interpreter TUnit suite now runs **1,862** tests in ~6.9 s (per the latest coverage run) with `FixtureCatalogGenerated.cs` tracking **87** fixtures. BinaryOperatorExpression, DescriptorHelpers, DynValue, DynamicExpression, FunctionCall, MemberDescriptor, Processor stack-trace, Io virtualization, Io module, PlatformAutoDetector, LuaIntegerHelper, PlatformAccessorBase, ModuleRegister, and PermanentRegistrationPolicy fixtures have been ported to TUnit, while `rg -l "[TestFixture]" src/tests/NovaSharp.Interpreter.Tests | Measure-Object` still reports **86** NUnit fixtures that must be retired.
- **2025-11-29 21:22 UTC update:** Ported `LimitedPlatformAccessorTests` to the new `LimitedPlatformAccessorTUnitTests` suite (under `src/tests/NovaSharp.Interpreter.Tests.TUnit/Platforms`) and removed the NUnit fixture. `scripts/tests/update-fixture-catalog.ps1` now reports **97** NUnit fixtures remaining, and `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` completed successfully to validate the conversion.
- **2025-11-29 21:45 UTC update:** Migrated `LuaStateInteropToolsTests` to `LuaStateInteropToolsTUnitTests` (under `src/tests/NovaSharp.Interpreter.Tests.TUnit/Units`), deleted the NUnit fixture, regenerated the catalog (now **96** remaining), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` to cover the new data-driven cases (total TUnit tests: **1,979**).
- **2025-11-29 22:10 UTC update:** Converted `MemberDescriptorTests` to `MemberDescriptorTUnitTests` (under `src/tests/NovaSharp.Interpreter.Tests.TUnit/Units`), refreshed `FixtureCatalogGenerated.cs` (now **96** fixtures referenced) with `rg -l "[TestFixture]" ...` down to **95**, and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` to verify the expanded suite (total TUnit tests: **1,990**).
- **2025-11-29 22:26 UTC update:** Ported `ProcessorStackTraceTests` to `ProcessorStackTraceTUnitTests` (under `src/tests/NovaSharp.Interpreter.Tests.TUnit/Units`), regenerated the catalog (now **95** fixtures) with `rg -l "[TestFixture]" ...` showing **94** NUnit fixtures outstanding, and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (total TUnit tests: **1,992**).
- **2025-11-29 21:45 UTC update:** Converted `ObjectCallbackMemberDescriptorTests` to `ObjectCallbackMemberDescriptorTUnitTests`, lowered the catalog to **94** fixtures with **93** NUnit suites remaining, and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (total TUnit tests: **1,994**).
- **2025-11-29 21:48 UTC update:** Removed the redundant NUnit `IoModuleVirtualizationTests` now that `IoModuleVirtualizationTUnitTests` covers the same scenarios, regenerated the catalog (now **93** fixtures with **92** NUnit suites outstanding), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (total TUnit tests unchanged at **1,994**).
- **2025-11-29 21:52 UTC update:** Removed `IoModuleTests` since `IoModuleTUnitTests` already covers the module’s entry points, regenerated the catalog (now **92** fixtures with **91** NUnit suites outstanding), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (total TUnit tests still **1,994**).
- **2025-11-29 21:56 UTC update:** Removed `PlatformAutoDetectorTests` because `PlatformAutoDetectorTUnitTests` already covers the detection logic, regenerated the catalog (now **91** fixtures with **90** NUnit suites outstanding), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (total TUnit tests remain **1,994**).
- **2025-11-29 21:53 UTC update:** Migrated `LuaIntegerHelperTests` to `LuaIntegerHelperTUnitTests`, regenerated the catalog (now **90** fixtures with **89** NUnit suites remaining), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (total TUnit tests: **1,999**).
- **2025-11-29 21:59 UTC update:** Ported `PlatformAccessorBaseTests` to `PlatformAccessorBaseTUnitTests`, deleted the NUnit fixture, regenerated the catalog (now **89** fixtures with **88** NUnit suites remaining), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (total TUnit tests: **2,007**).
- **2025-11-29 22:05 UTC update:** Migrated `ModuleRegisterTests` to `ModuleRegisterTUnitTests`, refreshed the catalog (now **88** fixtures with **87** NUnit suites remaining), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (total TUnit tests: **2,012**).
- **2025-11-29 22:09 UTC update:** Ported `PermanentRegistrationPolicyTests` to `PermanentRegistrationPolicyTUnitTests`, removed the NUnit fixture, regenerated the catalog (now **87** fixtures with **86** NUnit suites remaining), and reran `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` (total TUnit tests: **2,016**).
  - **Outstanding interpreter fixtures:** Focus next on the heaviest remaining NUnit suites—`ProcessorTests.cs`, the larger VM/process fixtures (`CoroutineCloseTests`, `CoroutineApiTests`, etc.), and the remaining module/tooling helpers under `Units/`. Tackling these batches will collapse the residual fixtures quickly so the interpreter NUnit project can be retired.
...
- 2025-11-29 17:21 UTC: Reran `./scripts/coverage/coverage.ps1` (Release suites: **1,862** TUnit + **1,309** NUnit) after fixing the new TUnit conversions. The run finished with **0** failures, refreshed `docs/coverage/latest/*` + `docs/coverage/coverage-hotspots.md`, and reports NovaSharp.Interpreter at **96.11 % line / 93.83 % branch / 97.43 % method**; `COVERAGE_GATING_MODE` stays in monitor mode until debugger/coroutine guard rails push branch coverage past the ≥95 % bar.
- Remote-debugger coverage regressed to **0.19 % line / 0 % branch** in this run, which means the remote debugger smoke tests are no longer being exercised during coverage. Re-enable the headless debugger fixtures (or add new TUnit coverage) so the debugger assemblies stay above the previously held 80 % targets.
