...
3. **Coverage Push (>90%)**
 ...
 - ✅ Drove `FileUserDataBase`/`StreamFileUserDataBase` coverage by stubbing stream failures (seek/flush/setvbuf) and verifying IO module fallbacks.
  ...
  - 🔜 Audit existing regression tests for spec fidelity: review each red/green test against the Lua 5.4 manual (and any documented deviations), catalog cases where we’re locking in behaviour that disagrees with stock Lua, and file follow-up bugs to bring NovaSharp back to spec instead of preserving incorrect MoonSharp-era expectations.
    - 🔄 Bootstrapped the audit log at `docs/testing/spec-audit.md`, citing Lua §3.4.1 for the fixed power-operator gap and flagging `EqualityTreatsNilAndVoidAsEqual` as “Needs investigation” so we confirm scripts cannot observe `nil == void`. Next pass: expand the table across other interpreter hotspots and file issues for every “Needs investigation” entry.
