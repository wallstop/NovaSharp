...
- **Progress snapshot (2025-11-30):** The interpreter TUnit suite now runs **2,373** tests in ~6 s (latest run at 14:20 UTC) with `FixtureCatalogGenerated.cs` tracking **43** NUnit fixtures. The Hardwire descriptor stack (code generation context, descriptor/member/method/userdata fixtures), DebugModule, LoadModule, LuaBase, HardwireParameter, CLI/REPL, os, and IO suites all live on TUnit, and `rg -l "[TestFixture]" src/tests/NovaSharp.Interpreter.Tests | Measure-Object` reports the remaining fixtures enumerated below.
