...
- 2025-12-03 16:05 UTC: Added `UserDataIsolationScope` plus a multi-lease helper inside `SemaphoreSlimScope` so tests can hold multiple semaphore slots via `SemaphoreSlimLeaseCollection`. `UserDataIsolationExecutor` now consumes the helpers, removing its last `try`/`finally` blocks while still honoring CA2007 via explicit `ConfigureAwait(false)`. Verified with `dotnet build src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- 2025-12-03 17:22 UTC: Introduced `ConsoleTestUtilities` to wrap `ConsoleCaptureScope`/`ConsoleRedirectionScope` instantiation and refactored the CLI + module/unit suites (`Cli/*`, `Modules/BasicModuleTUnitTests`, `Units/DotNetCorePlatformAccessorTUnitTests`) to depend on `WithConsoleCaptureAsync` / `WithConsoleRedirectionAsync` instead of manually calling `ConsoleCaptureCoordinator` or new-ing scopes. This centralizes console capture cleanup, lets the lint guards enforce the helper usage, and clears the remaining ad-hoc try/finally-free capture blocks. Validated via `dotnet build src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- 2025-12-03 17:55 UTC: Extended `scripts/lint/check-console-capture-semaphore.py` (and the `scripts/ci` wrapper + docs) so the guard now rejects both direct `ConsoleCaptureCoordinator.Semaphore` references and any `ConsoleCaptureScope`/`ConsoleRedirectionScope` instantiations outside `ConsoleTestUtilities`. This keeps new tests funnelled through the shared helpers and prevents future regressions.
- 2025-12-03 18:34 UTC: Added `TapStdinHelper` so TAP suites can pipe CRLF-backed stdin through NovaSharp without `io.popen`, patched `loadfile`/`loadfilesafe` to honor `stdin` when no filename is provided, tightened the CRLF handling in `FileUserDataBase` so `io.read('*number', ...)` no longer stops after the first line, and introduced an `IoModule` regression test that covers the CRLF scenario. The helper overrides `print` to capture stdout separately from diagnostics, and the updated lint guard now enforces helper usage for console capture. Verified with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- Next steps (carry-forward quality items):
  ...
  2. Keep `require "debug"` loading parity inside the TAP harness (`TestMore/LanguageExtensions/310-debug.t`) so the CLI runner mirrors vanilla Lua.
