...
- Progress (2025-11-26 11:54 UTC): converted BasicModule, Bit32Module, CoroutineModule, DebugModule, DynamicModule, ErrorHandlingModule, IoModule, JsonModule, LoadModule, MathModule, OsSystemModule, and OsTimeModule into static classes so the runtime no longer relies on CA1052 suppressions. ModuleRegister now registers those modules via `RegisterModuleType(table, typeof(...))`, and `dotnet build src/NovaSharp.sln -c Release -nologo` still completes with zero warnings.
...
- Remaining interpreter branch debt (per docs/coverage/latest/Summary.md, 2025-11-26 12:35 UTC): DynValue (~88.6 %), Coroutine (~83.3 %), DebugService (~83.3 %), SourceCode (~80 %), InterpreterException (~86.3 %), ParameterDescriptor (~87.5 %), NumericConversions (~89.1 %), CustomConverterRegistry (~87.5 %), Predefined EnumerableWrapper (~88.4 %), EventMemberDescriptor (~82 %), EmbeddedResourcesScriptLoader (~75 %), UnityAssetsScriptLoader (~86.8 %), ModuleRegister (~88 %), PlatformAutoDetector (~87.5 %), StandardPlatformAccessor (~89.2 %), ReplHistoryInterpreter (~87.5 %), ReplInterpreterScriptLoader (~87.5 %), Script (~83.8 %), SerializationExtensions (~89.6 %), UnaryOperatorExpression (~85 %), and PathSpanExtensions (~87.5 %). Prioritize these guard paths so branch coverage can cross ≥95 % and we can re-enable gating.
...
- Progress (2025-11-26 12:05 UTC): expanded `ModManifestTests` with JSON-null coverage so the parser/TryParse paths that previously short-circuited without branch coverage now assert the `"Manifest JSON resolved to an empty document."` failure. Validated the new tests with `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter "FullyQualifiedName~ModManifestTests"`. This removes an untested branch from the ModManifest hotspot list.
- Progress (2025-11-26 12:18 UTC): extended `ScriptRuntimeExceptionTests` to exercise the internal-error guard rails for `ArithmeticOnNonNumber` and `ConcatOnNonString`, ensuring the defensive “both operands valid” branches execute instead of remaining uncovered. Verified via `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter "FullyQualifiedName~ScriptRuntimeExceptionTests"`.
- Progress (2025-11-26 12:32 UTC): added null-guard and formatting coverage to `ScriptRuntimeExceptionTests` (null operands for arithmetic/concat/len/index/bitwise helpers plus the `BadArgumentUserData` branches that emit/omit the “nil or …” prefix and the null `got` formatting). Same targeted `dotnet test … --filter "FullyQualifiedName~ScriptRuntimeExceptionTests"` command keeps the guard clauses under regression and trims additional branch debt from `ScriptRuntimeException`.
- Progress (2025-11-26 12:44 UTC): expanded `DescriptorHelpersTests` with additional identifier/name fixtures (underscores-only camelify, trailing-digit snake-case, invalid characters, empty identifiers, generic conversion-name sanitization) so `DescriptorHelpers.NormalizeUppercaseRuns` and `ToUpperUnderscore` now exercise their remaining branches. Verified via `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter "FullyQualifiedName~DescriptorHelpersTests"`; this lifts `DescriptorHelpers` branch coverage out of the hotspot list.
- Progress (2025-11-26 12:56 UTC): broadened `LinkedListIndexTests` to cover the `_map == null` guard paths (Remove/TryGetValue/Contains/Clear) plus dictionary reuse after `Clear`, ensuring `LinkedListIndex<TKey, TValue>` now executes every branch. Verified via `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter "FullyQualifiedName~LinkedListIndexTests"` and removed the data-structure from the hotspot list.
- Progress (2025-11-26 13:05 UTC): added regression coverage for `Bit32Module.Bitwise` null guards and the private `NBitMask` saturation paths (zero/negative inputs and ≥32-bit masks) via reflection-backed NUnit tests. `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter "FullyQualifiedName~Bit32ModuleTests"` now exercises every bit32 branch, bumping the module above the ≥90 % branch threshold.
- Progress (2025-11-26 13:18 UTC): extended `IoModuleTests` with guard-rail coverage for `IoModule.SetDefaultFile` argument validation, the empty-mode path in `io.open`, and the public `IoExceptionToLuaMessage` helper (null exception + generic exception branches). Validated via `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter "FullyQualifiedName~IoModuleTests"` so `IoModule` inches past the ≥90 % branch target.
...
- Progress (2025-11-26 12:34 UTC): expanded `OsTimeModuleTests` with custom `ITimeProvider` fixtures (backwards clock, explicit timestamps), default-field coverage for `os.time`, and `%O`/`%E` specifier checks so `NovaSharp.Interpreter.CoreLib.OsTimeModule` now clears the ≥90 % branch threshold.
- Progress (2025-11-26 13:33 UTC): broadened `CoroutineTests` to cover the enumerable overloads, guard-path ArgumentNullExceptions, CLR-only error branches, `AutoYieldCounter`, and the internal test helpers so `NovaSharp.Interpreter.DataTypes.Coroutine` is no longer on the hotspot list. Verified via `dotnet test … --filter "FullyQualifiedName~CoroutineTests"`.
- Progress (2025-11-26 13:26 UTC): added `math.type`/`math.tointeger`/`math.ult` regression tests (including nil-return and argument-error branches) via `dotnet test src/tests/NovaSharp.Interpreter.Tests/NovaSharp.Interpreter.Tests.csproj -c Release --filter "FullyQualifiedName~MathModuleTests"` so `MathModule` now exceeds the ≥90 % branch threshold.
- Next steps: focus on the remaining high-priority hotspots (DynValue, DebugService/SourceCode, InterpreterException, ParameterDescriptor/NumericConversions, EventMemberDescriptor, and the Script/Repl/PathSpan helpers). Each needs dedicated unit tests covering the unhit guard paths so interpreter branch coverage can cross the ≥95 % enforcement bar.
