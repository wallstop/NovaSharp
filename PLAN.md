...
- 2025-12-01 19:37 UTC: NovaSharp.RemoteDebugger.Tests.TUnit now links the shared `TUnitAssertionConfigureAwaitExtensions`, adds the `TUnit.Assertions` package, and imports the test infrastructure via `GlobalUsings.cs`, letting `UtilitiesTUnitTests` apply `ConfigureAwait(false)` and drop its file-level CA2007 pragma. `dotnet test --project src/tests/NovaSharp.RemoteDebugger.Tests.TUnit/NovaSharp.RemoteDebugger.Tests.TUnit.csproj -c Release` passed after the update.
- 2025-12-01 19:39 UTC: `RegisterCommandTUnitTests` in the CLI suite now adopts `ConfigureAwait(false)` on every awaited assertion/console capture block, so its CA2007 pragma has been removed. The full interpreter TUnit suite (`dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`) passed to validate the change.
- 2025-12-01 19:40 UTC: `ReplInterpreterTUnitTests` now awaits all assertions with `ConfigureAwait(false)` and drops its file-level CA2007 suppression. Verified with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- 2025-12-01 19:42 UTC: `ReplHistoryInterpreterTUnitTests` has also been converted to `ConfigureAwait(false)` for every awaited assertion, eliminating another CLI CA2007 pragma. `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` was rerun (initial attempt hit the flaky `ScriptRunTUnitTests.RunFileThrowsWhenFileMissing`, second pass succeeded) to confirm the update.
- 2025-12-01 19:44 UTC: `DebugCommandTUnitTests` now awaits every assertion/console capture call via `ConfigureAwait(false)`, so the CLI debugger fixture’s CA2007 pragma is gone. Revalidated with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- 2025-12-01 19:50 UTC: `HardwireCommandTUnitTests` adopts `ConfigureAwait(false)` for all assertions, console captures, and semaphore scopes (the async disposal path now uses `ConfiguredAsyncDisposable`), allowing its CA2007 pragma to be removed. Verified with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- 2025-12-01 19:55 UTC: `CliIntegrationTUnitTests` now applies `ConfigureAwait(false)` to all assertions, `WithConsoleAsync` callers, and semaphore leases (mirroring the new `ConfiguredAsyncDisposable` pattern), so the suite no longer needs the CA2007 pragma. Confirmed with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- 2025-12-01 20:12 UTC: Began sweeping the module suites by updating `BasicModuleTUnitTests` to use `ConfigureAwait(false)` on all assertions so its pragma could be removed; validated with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- 2025-12-01 20:15 UTC: `MathModuleTUnitTests` now runs its Lua math coverage with `ConfigureAwait(false)` on every assertion, eliminating another module pragma. `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` confirmed the update.
- 2025-12-01 21:14 UTC: `StringModuleTUnitTests` applies `ConfigureAwait(false)` across its string API assertions and no longer needs CA2007 suppression. Revalidated with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- 2025-12-01 21:16 UTC: `TableModuleTUnitTests` now uses `ConfigureAwait(false)` on every assertion inside its table API coverage, clearing another module-level CA2007 pragma; `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` passed afterward.
- 2025-12-01 21:25 UTC: `CoroutineModuleTUnitTests` has been converted to `ConfigureAwait(false)` throughout (including the async background resume case), eliminating its CA2007 pragma. `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` validated the change.
- 2025-12-01 21:29 UTC: `DebugModuleTUnitTests` now awaits all debug API assertions with `ConfigureAwait(false)` and no longer carries a CA2007 pragma; a fresh interpreter TUnit run confirmed the refactor.
- 2025-12-01 21:31 UTC: `JsonModuleTUnitTests` awaits every encode/decode assertion via `ConfigureAwait(false)` so its CA2007 pragma could be deleted; `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` passed.
- 2025-12-01 21:33 UTC: `OsTimeModuleTUnitTests` now applies `ConfigureAwait(false)` across all time/clock/date assertions, dropping another module pragma. Verified with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- 2025-12-01 21:42 UTC: `LoadModuleTUnitTests` adopts `ConfigureAwait(false)` for its require/loadfile/dofile assertions, so the module no longer needs a CA2007 pragma. `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` confirmed the change.
- 2025-12-01 21:44 UTC: `LuaBaseTUnitTests` now awaits every Lua base interop assertion via `ConfigureAwait(false)` and drops its CA2007 pragma; `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` passed.
- 2025-12-01 22:47 UTC: `Bit32ModuleTUnitTests` has been converted to `ConfigureAwait(false)` end-to-end, removing its CA2007 pragma. `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release` passed afterward.
- 2025-12-01 22:49 UTC: `StandardIoFileUserDataBaseTUnitTests` now awaits its stream assertions with `ConfigureAwait(false)`, eliminating another module-level CA2007 pragma. Validated with `dotnet test --project src/tests/NovaSharp.Interpreter.Tests.TUnit/NovaSharp.Interpreter.Tests.TUnit.csproj -c Release`.
- Ease-of-use backlog (still active):
  ...
  3. CustomConverterScope for managing Script.GlobalOptions.CustomConverters. ✅ (2025-12-01 23:05 UTC)
