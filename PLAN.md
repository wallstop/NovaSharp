...
- Progress snapshot (2025-11-29):
  ...
  - 2025-11-29: Ported `EndToEnd/UserDataEventsTests` to `EndToEnd/UserDataEventsTUnitTests` (async event handler wiring, static vs. instance events, detach/reregister paths) and confirmed the TUnit project still builds cleanly. Interpreter-wide `dotnet test` attempts remain blocked by the previously documented collections/user-data descriptor failures, so the new event coverage currently only compiles.
