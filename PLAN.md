...
- Progress to date:
  ...
  - Completed TUnit suites: remote-debugger pilot, ScriptLoad, ScriptRun, ScriptCall, ScriptExecutionContext, ScriptLoaderBase, ScriptDefaultOptions, ScriptPrivateResourceExtension (artefacts kept in the migration folder). These no longer need explicit checkpoints in PLAN.md.
...
- **Next steps (2025-11-27 update):**
  1. Continue the VM/core migration by targeting the remaining Script fixtures (`ScriptPrivateResourceExtension` now complete; next up: e.g., `ScriptTypeMetatableTests`, `ScriptLoaderBase`-adjacent helpers) and log compare artefacts for each batch.
