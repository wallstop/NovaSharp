...
| To-be-closed variables (`local f <close> = ...`) | ✅     | `CloseAttributeTests`, `CoroutineCloseTests`                 | Reverse-order closing, reassignment, coroutine yield/close flows, and error propagation now execute the registered `__close` metamethods. TAP `310-close-var.t` mirrors remain temporarily disabled while `require` loader bugs are debugged; NUnit mirrors keep the coverage live. Owner: Interpreter. |
