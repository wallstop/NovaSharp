...
| `io.read("*n")` hex/exponent parsing | ⚠️ | Current runtime consumes only decimal prefixes (returns `0` for `0x` literals and `math.huge` for overflows). Lua 5.4.8 supports hexadecimal `p` exponents and leaves unread suffixes. Tracked via skipped NUnit tests in `IoModuleTests`. |
