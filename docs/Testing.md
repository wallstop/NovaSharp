...
- Pull requests receive a sticky aggregate Lua comparison comment generated by `scripts/tests/render-lua-comparison-report.py`. The comment summarizes every uploaded matrix artifact, including failed matrix legs, so reviewers can distinguish production regressions from fragile or unnecessary fixture failures without digging through each job.
