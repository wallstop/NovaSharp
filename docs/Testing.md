...
- Tests must never call `Path.GetTempPath()` directly anymore—use `TempFileScope`/`TempDirectoryScope` so cleanup stays centralized. `python scripts/lint/check-temp-path-usage.py` (or `./scripts/ci/check-temp-path-usage.sh`) enforces this by failing whenever a new test references `Path.GetTempPath()` outside the shared scope helper.
