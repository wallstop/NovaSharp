...
2. **Document scripts**: if you add a script under `scripts/`, extend `scripts/README.md` (and the relevant subfolder README) plus any guide that references the workflow (most commonly `docs/Testing.md` or `docs/Modernization.md`).
3. **Cross-link related docs**: for example, when editing coverage processes, update this index, `docs/Testing.md`, and the relevant plan entries so readers land on current instructions quickly.
