...
| `LuaPort/LuaStateInterop/CharPtr.cs`, `LuaBase.cs`, `LuaBaseCLib.cs`, `LuaLBuffer.cs`, `LuaState.cs`, `Tools.cs` | These files mirror the Lua C API surface so host integrations (Unity, hardwire generators) can consume the same constants/method names when running mixed C#/Lua workloads. Leaving identifiers like `LUA_MULTRET`, `lua_pushvalue`, or `luaopen_base` untouched keeps the mental model aligned with the upstream manual and avoids churn when syncing bug fixes from KopiLua. | The entire `NovaSharp.Interpreter.LuaPort.LuaStateInterop` namespace is excluded from IDE1006 in `.editorconfig`. Any new helpers added outside this namespace must follow the normal PascalCase/CamelCase rules. |
