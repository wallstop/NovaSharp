...
NovaSharp inherited a mix of naming conventions from the legacy runtime (e.g., Hungarian-style member prefixes, `Emit_*` helpers). This audit captures the current hotspots and is authoritative for detailed naming-alignment work.
...
Tracking: file issues for actionable hotspots and place only the selected next alignment in `PLAN.md`.
