...
2. Prototype `Table.FillPairs` + pooled list usage in interpreter tests; measure allocation delta.
3. Evaluate struct-based disposable pattern compatibility (ensure existing consumers compiling against `IDisposable` continue to work).
4. Document findings + adoption guidance in `docs/Performance.md` before rolling out API changes.
5. Update PLAN.md as experiments land (per area).
