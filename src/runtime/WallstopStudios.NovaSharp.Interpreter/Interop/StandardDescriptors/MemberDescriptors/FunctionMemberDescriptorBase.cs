namespace WallstopStudios.NovaSharp.Interpreter.Interop.StandardDescriptors.MemberDescriptors
{
    ...
    public abstract class FunctionMemberDescriptorBase : IOverloadableMemberDescriptor
    {
        ...
        )
        {
            ...
            {
                SortDiscriminant = ZStringBuilder.Join(':', GetParameterTypeNames(_parameters));
            }
        }

        private static string[] GetParameterTypeNames(ParameterDescriptor[] parameters)
        {
            string[] names = new string[parameters.Length];
            for (int i = 0; i < parameters.Length; i++)
            {
                names[i] = parameters[i].Type.FullName ?? string.Empty;
            }
            return names;
        }
