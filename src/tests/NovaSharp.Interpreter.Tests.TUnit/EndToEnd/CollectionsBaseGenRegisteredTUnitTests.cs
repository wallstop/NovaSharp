    ...
    public sealed class CollectionsBaseGenRegisteredTUnitTests
    {
        ...
        public Task RegCollGenIteratorOnMultiDimArrayChangeElem()
        {
            return RunScriptAsync(
                ...
                {
                    await Assert.That(result.Type).IsEqualTo(DataType.Number).ConfigureAwait(false);
                    await Assert.That(result.Number).IsEqualTo(41).ConfigureAwait(false);
                    await Assert
                        .That(host.GetMultiArray()[0, 0])
                        .IsEqualTo(2)
                    ...
                    await Assert
                        ...
                        .ConfigureAwait(false);
                    await Assert
                        .That(host.GetMultiArray()[0, 2])
                        .IsEqualTo(6)
                        .ConfigureAwait(false);
                    await Assert
                        .That(host.GetMultiArray()[1, 0])
                        .IsEqualTo(7)
                        .ConfigureAwait(false);
                    await Assert
                        .That(host.GetMultiArray()[1, 1])
                        .IsEqualTo(8)
                        .ConfigureAwait(false);
                    await Assert
                        .That(host.GetMultiArray()[1, 2])
                        .IsEqualTo(9)
                        .ConfigureAwait(false);
                }
            );
