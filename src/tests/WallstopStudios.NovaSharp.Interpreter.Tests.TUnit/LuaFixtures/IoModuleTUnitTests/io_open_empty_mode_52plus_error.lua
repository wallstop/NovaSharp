-- @lua-versions: 5.2+
-- @novasharp-only: false
-- @expects-error: false
-- @source: src/tests/WallstopStudios.NovaSharp.Interpreter.Tests.TUnit/Modules/IoModuleTUnitTests.cs
-- @test: IoModuleTUnitTests.OpenThrowsWhenModeEmptyLua52Plus

-- Test: io.open throws error for empty mode in Lua 5.2+
-- Reference: Lua 5.2+ manual - io.open
-- Lua 5.1 returns (nil, error) for empty mode; Lua 5.2+ throws error

local ok, err = pcall(function()
    return io.open("/tmp/test_file.txt", "")
end)
print("throws error:", not ok)
print("error contains 'invalid mode':", (err or ""):find("invalid mode") ~= nil)
return not ok
