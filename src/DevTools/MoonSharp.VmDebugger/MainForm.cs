...
namespace MoonSharp.Debugger
{
    public partial class MainForm : Form, IDebugger
    {
        ...
        private void DebugScript(string filename)
        {
            ...
            m_Script.Options.DebugPrint = s =>
            {
                Console_WriteLine("{0}", s);
            };
        ...
        {
            string[] source = byteCode
                .Select((s, i) => string.Format("{0:X8}  {1}", i, s))
                .ToArray();
        ...
        private void RefreshCodeView(SourceRef sourceCodeRef)
        {
            ...
            else
            {
                ...
                txtCodeView.Text =
                    sc.GetCodeSnippet(sourceCodeRef) + "\r\n\r\n" + sourceCodeRef.ToString();
            }
        }
        ...
        void DebugMain(DynValue fn)
        {
            ...
            catch (ScriptRuntimeException ex)
            {
                timerFollow.Enabled = false;
                Console_WriteLine(
                    "Guest raised unhandled CLR exception: {0} -@{3:X8} {2}\n{1}\n",
                    ex.GetType(),
                    ex.ToString(),
                    ex.DecoratedMessage,
                    ex.InstructionPtr
            ...
            catch (Exception ex)
            {
                timerFollow.Enabled = false;
                Console_WriteLine(
                    "Guest raised unhandled CLR exception: {0} \n{1}\n",
                    ex.GetType(),
                    ex.ToString()
                );
            }
        ...
        {
            DebugAction(
                new DebuggerAction() { Action = DebuggerAction.ActionType.ByteCodeStepOver }
            );
        }
        ...
        private void btnAddWatch_Click(object sender, EventArgs e)
        {
            ...
            {
                string[] codeToAdd = text.Split(
                    new char[] { ',' },
                    StringSplitOptions.RemoveEmptyEntries
                );

                m_Watches.AddRange(
                    codeToAdd.Select(code => m_Script.CreateDynamicExpression(code))
                );
        ...
        private void btnRemoveWatch_Click(object sender, EventArgs e)
        {
            HashSet<string> itemsToRemove = new HashSet<string>(
                lvWatches.SelectedItems.OfType<ListViewItem>().Select(lvi => lvi.Text)
            );
