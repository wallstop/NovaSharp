...
namespace MoonSharp.Interpreter.Interop
{
    ...
    public class PropertyTableAssigner : IPropertyTableAssigner
    {
        ...
        public PropertyTableAssigner(Type type, params string[] expectedMissingProperties)
        {
            ...
            {
                foreach (
                    MoonSharpPropertyAttribute attr in pi.GetCustomAttributes(true)
                        .OfType<MoonSharpPropertyAttribute>()
                )
                {
                    ...
                    if (m_PropertyMap.ContainsKey(name))
                    {
                        throw new ArgumentException(
                            string.Format(
                                "Type {0} has two definitions for MoonSharp property {1}",
                                m_Type.FullName,
                                name
                            )
                        );
        ...
        private void AssignProperty(object obj, string name, DynValue value)
        {
            if (TryAssignProperty(obj, name, value))
                return;
            if (TryAssignProperty(obj, DescriptorHelpers.UpperFirstLetter(name), value))
                return;
            if (TryAssignProperty(obj, DescriptorHelpers.Camelify(name), value))
                return;
            if (
                TryAssignProperty(
                    obj,
