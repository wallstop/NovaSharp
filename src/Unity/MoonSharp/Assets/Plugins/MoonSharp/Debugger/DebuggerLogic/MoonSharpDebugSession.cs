...
namespace MoonSharp.VsCodeDebugger.DebuggerLogic
{
    internal class MoonSharpDebugSession : DebugSession, IAsyncDebuggerClient
    {
...
#if DOTNET_CORE
            SendText(
                "Connected to MoonSharp {0} [{1}]",
                Script.VERSION,
                Script.GlobalOptions.Platform.GetPlatformName()
            );
#else
            SendText(
                "Connected to MoonSharp {0} [{1}] on process {2} (PID {3})",
                Script.VERSION,
                Script.GlobalOptions.Platform.GetPlatformName(),
        ...
        public override void Evaluate(Response response, Table args)
        {
            ...
            SendResponse(
                response,
                new EvaluateResponseBody(v.ToDebugPrintString(), m_Variables.Count - 1)
                {
        ...
        private void ExecuteRepl(string cmd)
        {
            ...
            else if (cmd.StartsWith("execendnotify"))
            {
                ...
                SendText(
                    ...
                    m_NotifyExecutionEnd ? "enabled" : "disabled"
                );
            }
            else if (cmd == "list")
            {
