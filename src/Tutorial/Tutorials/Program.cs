    ...
    class Program
    {
        static void Main(string[] args)
        {
            while (true)
            {
                ...
                List<Tuple<string, MethodInfo>> tutorialMethods =
                    new List<Tuple<string, MethodInfo>>();

                foreach (
                    MethodInfo mi in chosenType
                        .GetMethods(
                            BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public
                        )
                        .Where(mi => mi.GetCustomAttributes(typeof(TutorialAttribute)).Any())
                )
                {
                    tutorialMethods.Add(new Tuple<string, MethodInfo>(mi.Name, mi));
                }

                MethodInfo chosenMethod = DoMenu(
                    string.Format("{0} - Choose tutorial", chosenType),
                    tutorialMethods
                );
        ...
        private static T DoMenu<T>(string title, List<Tuple<string, T>> choices)
        {
            while (true)
            {
                ...
                if (int.TryParse(choice, out chosen))
                {
                    if (chosen == 99)
                        return default(T);
                    if (chosen >= 1 && chosen <= choices.Count)
                        return choices[chosen - 1].Item2;
