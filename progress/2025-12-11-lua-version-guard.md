...
1. **`IsMemberCompatible()`** in `ModuleRegister.cs` filters functions at registration time
1. Functions not available in the current Lua version are simply not registered
...
1. **Runtime checks** when the function exists but has version-specific behavior
1. **Guard conditions** in complex logic that depends on version availability
1. **Documentation** of version requirements in code
...
1. **§8.41 (Lua Version Compatibility Audit)**:
