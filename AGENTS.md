...
- Run the detector/console-capture/try-finally lint guards (`python scripts/lint/check-platform-testhooks.py`, `python scripts/lint/check-console-capture-semaphore.py`, and `python scripts/lint/check-test-finally.py`) before pushing so new tests never reference `PlatformAutoDetector.TestHooks`, `ConsoleCaptureCoordinator.Semaphore`, or raw `finally` blocks. CI runs the same scripts via their `scripts/ci/*` wrappers.
