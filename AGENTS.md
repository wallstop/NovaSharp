...
- **PRODUCTION BUG POLICY**: Never adjust tests to accommodate production bugs. When a test fails or exposes incorrect behavior, assume production is wrong until proven otherwise. Fix the production code to produce correct output, keep tests unchanged unless demonstrably incorrect, and verify against specifications (Lua manual, DAP protocol spec, etc.).
