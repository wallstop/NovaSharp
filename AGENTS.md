...
- Fixtures that mutate shared/static state (anything that calls `UserData.RegisterType`, tweaks the registration policy, etc.) must include `[UserDataIsolation]` so the registry is sandboxed per test. If a suite manipulates `Script.GlobalOptions` (custom converters, platform hooks, etc.), decorate it with `[ScriptGlobalOptionsIsolation]` (or open a scope in `[SetUp]`/`[TearDown]` when the attribute is insufficient) so those tweaks stay local.
...
- Run the detector and console-capture lint guards (`python scripts/lint/check-platform-testhooks.py` and `python scripts/lint/check-console-capture-semaphore.py`) before pushing so new tests never reference `PlatformAutoDetector.TestHooks` or `ConsoleCaptureCoordinator.Semaphore` directly. CI runs the same scripts via `scripts/ci/check-platform-testhooks.sh` / `check-console-capture-semaphore.sh`.
