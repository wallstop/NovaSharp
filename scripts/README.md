...
- `benchmarks/` — Helpers for running the runtime + comparison BenchmarkDotNet suites (`run-benchmarks.ps1` / `run-benchmarks.sh`), keeping `docs/Performance.md` up to date, and rendering `artifacts/benchmark-deltas.md` against the frozen MoonSharp baseline. CI integration via `.github/workflows/benchmarks.yml` tracks regressions with threshold-based alerting and PR comments.
