...
- `check_jekyll_liquid.py` — Fails when any Markdown file GitHub Pages would render contains Liquid syntax that aborts the site build. Pages serves this repository through the `github-pages` gem, whose `jekyll-optional-front-matter` plugin turns every published Markdown file into a Jekyll page, so a Lua nested table constructor written without inner spaces reads as an unterminated Liquid variable and takes the whole site down. The check reproduces `Liquid::BlockBody`'s fatal cases (unterminated variable and tag delimiters, unknown tags, and an unclosed raw region) and was validated case-by-case against a real `github-pages` v232 build — 18/18 verdicts matched Jekyll's exit code. It derives its scan set from `_config.yml`'s `exclude` list, so the guard covers exactly what Pages publishes; excluding a path there removes it from both at once.
...
- `check-tooling-consistency.sh` — Runs `scripts/lint/check-tooling-consistency.py` to keep devcontainer SDK packages, local .NET tool usage, and hook restore behavior aligned with `global.json` and `.config/dotnet-tools.json`.
